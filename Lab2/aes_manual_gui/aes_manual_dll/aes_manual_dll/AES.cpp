...
// Constructor
AES::AES(const std::vector<uint8_t>& key, size_t key_length) : key(key), key_length(key_length)
{
    S_BOX = {
...
// Private member function: Substitution using S-box
std::vector<uint8_t> AES::sub_word(std::vector<uint8_t> word)
{
...
// Private member function: Rotates the word by one byte to the left
std::vector<uint8_t> AES::rot_word(std::vector<uint8_t> word)
{
    std::vector<uint8_t> result;
...
// Private member function: Key expansion
std::vector<std::vector<uint8_t>> AES::key_expansion_128()
{
    int key_size = 16;
    int key_words = 4;
    round_keys = generateRoundKeys(key, key_size);
    for (int i = 4; i < 44; i++)
    {
...
std::vector<std::vector<uint8_t>> AES::key_expansion()
{
    ...
    {
        cerr << "Invalid key length. Supported lengths are 128, 192, and 256 bits.";
        exit(0);
    }
}
...
void AES::inv_mix_columns(std::vector<std::vector<uint8_t>>& state)
{
...
void AES::inv_shift_rows(std::vector<std::vector<uint8_t>>& state)
{
...
// Public member function: Encrypts the input data
...
// Public member function: Decrypts the input ciphertext
